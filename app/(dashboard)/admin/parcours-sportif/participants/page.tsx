...
const PAYMENT_LABELS: Record<string, string> = {
  ...
  free: 'Gratuit', waived: 'Offert', cancelled_admin: 'Annulée',
}
const PAYMENT_COLORS: Record<string, string> = {
  ...
  waived: 'bg-purple-100 text-purple-700', cancelled_admin: 'bg-gray-100 text-gray-400',
}
...
function buildParticipants(registrations: Registration[]): Participant[] {
  ...
  for (const r of registrations) {
    ...
    if (!existing) {
      map.set(key, {
        ...
        pendingCount: r.paymentStatus === 'pending' ? 1 : 0,
      })
    } else {
      ...
      if (r.paymentStatus === 'pending') existing.pendingCount++
    }
  }
...
export default function ParticipantsPage() {
  ...
  const [filterChip, setFilterChip] = useState<'all' | 'present' | 'absent' | 'pending' | 'homonyms'>('all')
  const [sortBy, setSortBy] = useState<'name' | 'mostSessions' | 'mostPresent' | 'mostAbsent'>('name')

  // Detail panel
  const [selected, setSelected] = useState<Participant | null>(null)
  const [detailTab, setDetailTab] = useState<'sessions' | 'stats'>('sessions')
  ...
  const participants = useMemo(() => buildParticipants(registrations), [registrations])
  ...
  }, [participants, search, filterChip, sortBy, homonymKeys])
                ...
                <div className="flex border-b border-gray-100">
                  {(['sessions', 'stats'] as const).map((t) => (
                    <button key={t} onClick={() => setDetailTab(t)}
                      ...
                      {t === 'sessions' ? 'Séances' : 'Statistiques'}
                    </button>
                  ))}
                </div>
